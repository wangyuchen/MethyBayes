...
#' @param data a list of read counts, methylated read counts and commonly mapped sites
...
process<-function(data, n1, n2){
  ...
  s<-1000
  for(i in 1:L){
    x<-rbeta(s,alp1[i],bta1[i])
    y<-rbeta(s,alp2[i],bta2[i])
  ...
  for(i in 1:L){
    ...
    p[2,i]<-as.numeric(lklihd2(CDcol,CUcol,MDcol,MUcol,integ1, alpha2,beta2))
    p[3,i]<-as.numeric(lklihd3(CDcol,CUcol,MDcol,MUcol,integ2, alpha3,beta3))
  }
